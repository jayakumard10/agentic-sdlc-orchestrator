...
| Process/planning notes could accidentally end up in the public repo | Local planning notes (`PLANNING.md`, containing the full implementation plan and a running deviation log) live in this same working directory but are gitignored and were never committed at any point in this repo's history — confirmed via `git log`/`git status` throughout the build, not just at the end. |
| The target app is an open redirect by design | `POST /shorten` accepts any syntactically valid HTTP(S) URL (verified directly: Pydantic's `HttpUrl` does not reject private IPs or cloud-metadata addresses like `169.254.169.254`), and `GET /{code}` issues a real redirect to whatever was stored. This is not SSRF — nothing server-side fetches the URL, the redirect is a client-side browser navigation — and it's an accepted, industry-standard property of URL shorteners generally (the same is true of bit.ly/tinyurl). Not mitigated; called out here as a known, accepted risk rather than an oversight, since a production deployment might additionally choose to blocklist private/link-local destination ranges depending on threat model. |
