...
def capture(scenario_type: str) -> None:
    ...
    decisions = CAPTURE_DECISIONS[scenario_type]
    workspace = seed_workspace(scenario_type, _CAPTURE_WORKSPACE_ROOT)
    ...
    result = compiled.invoke(
        GraphState(scenario_type=scenario_type, requirement_raw=requirement_raw, mode="live"),
        config=config,
    )
    ...
    if "__interrupt__" in result:
        raise RuntimeError(
            ...
            "a gate type is missing from CAPTURE_DECISIONS[...]"
        )
    ...
    out_dir = FIXTURES_DIR / scenario_type
...
if __name__ == "__main__":
    if len(sys.argv) != 2 or sys.argv[1] not in SCENARIO_REQUIREMENTS:
        print(
            f"usage: python {sys.argv[0]} <{'|'.join(SCENARIO_REQUIREMENTS)}>",
            file=sys.stderr,
        )
        raise SystemExit(1)
    capture(sys.argv[1])
